...
pub fn build_apps(
    ...
    for item in &resolved.build.apps {
        let crate_dir = Path::new(&item.path).parent().ok_or_else(|| {
            "app path must point to a valid Cargo.toml".to_string()
        })?;
        ...
        *current_step += 1;
        print!("  Compiling ({}/{}) app {}...", comp_step, total_steps, crate_name);
        ...
        if !result.success {
            println!();
            return Err(format!("failed to build app {}", crate_name));
        }
        println!("\r  Compiling ({}/{}) app {}... DONE\x1B[K", comp_step, total_steps, crate_name);
        ...
        *current_step += 1;
        ...
        if !result.success {
            println!();
            return Err(format!("failed to pack app {}", bin_name));
        }
