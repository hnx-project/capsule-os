...
| **A2** | EL0-FAULT during `sys_spawn(devmgr)` | `working` | **CRITICAL ARCHITECTURE BUGFIX**: This long-standing trap (Instruction Abort 0x86000005, Data Abort 0x96000005) is resolved. The investigation uncovered three critical microkernel design issues: (1) **HandleTable Size-Boundary Overflow**: With MAX_HANDLES=32, the size of HandleTable exceeded 4 KiB and crossed into unmapped discontinuous physical pages, which corrupted table references when index overflowed; resolved by constraining MAX_HANDLES to 8 to fit within a single 4 KiB page. (2) **Identity Map Lost in User Page Tables**: The kernel is linked at the physical low-half region (0x40080000), meaning all global statics/v-tables (e.g. ConsoleWriter/Logger) use low-half physical addresses. Switching TTBR0_EL1 to user L0 page tables unmapped the low-half kernel space, causing Translation Faults during syscall logging; resolved by copying L0 entry 0 (the identity-mapped low-half entry) into all new user page directories. (3) **4 KiB Stack Allocation**: The previous sys_load_binary allocated `tmp: [u8; 4096]` on the limited 16 KiB kernel stack, which smashed stack frames; resolved by directly streaming VMO bytes into the global .bss scratch slice. |
