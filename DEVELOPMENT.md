...
*   **`posix_spawn(3)` is the recommended spawn API.**  See `libraries/libcapsule/src/posix_spawn/API.md`.  The 1.0 implementation honours `adddup2` into fd 0/1/2 and `POSIX_SPAWN_WAITPID`; `addopen` / `addclose` are recorded but not yet executed by the kernel-side loader.  Real structured `argv` / `envp` forwarding lands under S13.
...
*   **Spawn children via `posix_spawn(3)`, not `fork()`**: New processes are created through `posix_spawn(3)` (Fuchsia / Zircon model).  `libc::fork()` is `ENOSYS` at the libc boundary; see §5 for the rationale.  See `libraries/libcapsule/src/posix_spawn/API.md` for the 1.0 surface (file actions `adddup2` only; `addopen`/`addclose` recorded but not executed).
